...
const int HASH_TABLE_SIZE = 2179000; //9971; //A prime roughly 10% larger
...
Edge* Edges=new Edge[ HASH_TABLE_SIZE ];
...
//
int* CompletedEdges=new int[ HASH_TABLE_SIZE];
int Node::Count = 1;
Node* Nodes=new Node[ MAX_LENGTH * 2 ];
int size;
int string_size;
...
char* string_input=new char[ MAX_LENGTH];
char* string_in=new char[ MAX_LENGTH];
int current_end;
int start = 0;
int entity_count = 0;
int N;
...
void search_by_entity(int cluster_id,int doc_id);
...
TreeEntity* TreeEntities=new TreeEntity[MAX_LENGTH * 2];
//This type of array is ultimately kept
TreeEntity EligibleEntities[2][MAX_LENGTH * 2];
...
void show(int current_n) {
    ...
    for (int j = 0; j < HASH_TABLE_SIZE; j++) {
        ...
        cout << setw(10) << Nodes[s->end_node].isLeaf << " ";
        cout << setw(10) << Nodes[s->end_node].termination_label << "  ";
        cout << setw(10) << EligibleEntities[cluster_id][s->end_node].document_id << "  ";
        cout << "\n";
        cout << TreeEntities[s->end_node ].end_node
                << ","
                << TreeEntities[s->end_node ].depth
...
void insert_document_api(char string_input[ MAX_LENGTH ], int docId) {
    ...
    insert_document(active, string_input, docId);
}
...
//To insert document
    ...
    for (int y = 0; y < 2; y++){
        refresh();
      cout << "Here Tree number 2";
    p = 0;
    for (int x = 0; x < 3; x++) {
        //while(true){
        delete[] string_in;
        string_in = new char[MAX_LENGTH];
        cout << "PRE3 STRING "
                << string_in;
        ...
        cout << "PRE STRING "
                << string_in;
        string_in[strlen(string_in)] = '$';
        line_length++;
        cout << "STRING "
                << string_in
                << "MAIN STREME "
                << T;
        insert_document_api(string_in, x);
    ...
    search_by_entity(0,1);
    search_by_entity(0,2);
...
void store(Suffix active, char string_input[ MAX_LENGTH]) {
   ...
   // if (doc_count == 1) {
       ...
       // T[N]='*';
//    } else {
//       // active.first_char_index=Edges[0].first_char_index;
//       // active.last_char_index=Edges[0].last_char_index;
...
void search_by_entity(int cluster_id,int doc_id) {
    ...
    for (int m = 0; m < 1000000
        ...
        if (EligibleEntities[cluster_id][m].document_id == doc_id) {

            eligible_entity_list.push_back(EligibleEntities[cluster_id][m]);


            cout << ",,,,,,,,,,,,,,,,,,"
                    << EligibleEntities[cluster_id][m].termination_char;
        }
    ...
    cout << "Here are the strings :";
    k = entity.end_node;
    next = EligibleEntities[cluster_id][k];

    cout << k
            << next.sequence;
    while (k != 0) {

        k = EligibleEntities[cluster_id][k].parent;
        next = EligibleEntities[cluster_id][k];
...
void store_eligible_entities(int docId) {
    ...
    for (std::list<char>::iterator it = terminators.begin(); it != terminators.end(); it++) {
        ...
        k = entity.end_node;
        next = TreeEntities[k];
        EligibleEntities[cluster_id][k] = TreeEntities[k];
        EligibleEntities[cluster_id][k].document_id = docId;
        cout << k
                << next.sequence;
        while (k != 0) {
    ...
    for (int b = 0; b < 1000000; b++) {
        length_final += EligibleEntities[cluster_id][b].sequence.length();
    }
...
void refresh(){
   Edges = new Edge[ HASH_TABLE_SIZE ];
        ...
        CompletedEdges = new int[ HASH_TABLE_SIZE];
        //Node::Count = 1;
        Nodes = new Node[ MAX_LENGTH * 2 ];
        size = 0;
        string_size = 0;
        required = new int[100];
        temp_document_count = 0;
        full_document_count = 0;
        temp_hashtable_size = 0;
        doc_count = 0;
        T = new char[ MAX_LENGTH ];
        string_input = new char[ MAX_LENGTH];
        string_in = new char[ MAX_LENGTH];
        current_end = 0;
        start = 0;
        entity_count = 0;
        N = 0;
        TreeEntities = new TreeEntity[MAX_LENGTH * 2];
